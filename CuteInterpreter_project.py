...
class CuteInterpreter(object):
    ...
    def run_arith(self, arith_node):
        ...
        if arith_node.type is TokenType.PLUS:
            ...
            expr_rhs1 = self.run_expr(rhs1)
            if expr_rhs1.value > expr_rhs1.next.value:
            ...
            expr_rhs1 = self.run_expr(rhs1)
            if expr_rhs1.value is expr_rhs1.next.value:
                return self.TRUE_NODE
            return self.FALSE_NODE
    ...
    def run_func(self, func_node):
        ...
        if func_node.type is TokenType.CAR:
            ...
            if not is_quote_list(rhs1):
                print ("cdr error!")
            result = pop_node_from_quote_list(rhs1)
            a= create_quote_node(result.next, True)
            return  a
            ...
            if expr_rhs1.type is TokenType.LIST:
                a = expr_rhs1.value.next
            else:
                a = expr_rhs1
            result = pop_node_from_quote_list(expr_rhs2)
            a.next = result
    ...
    def cond_fun(self,cond_node):
        if cond_node.value.type is TokenType.LIST:
            if self.run_expr(cond_node.value).type is TokenType.TRUE:
                return cond_node.value.next
            elif self.run_expr(cond_node.value).type is TokenType.FALSE:
                return self.cond_fun(cond_node.next)
        elif cond_node.value.type is TokenType.TRUE:
            return cond_node.value.next
    ...
    def run_expr(self, root_node):
        ...
        elif root_node.type is TokenType.PLUS:
            return root_node
        elif root_node.type is TokenType.MINUS:
            return root_node
        elif root_node.type is TokenType.COND:
            return root_node
        elif root_node.type is TokenType.GT:
            return root_node
        elif root_node.type is TokenType.EQ:
            return root_node
        elif root_node.type is TokenType.NOT:
            return root_node
        else:
    ...
    def run_list(self, l_node):
        ...
        if op_code.type in \
                [TokenType.PLUS, TokenType.MINUS, TokenType.TIMES, TokenType.DIV, TokenType.GT, TokenType.LT, TokenType.EQ]:
